...
class PCBuildersCrawler(BaseCrawler):
    def __init__(self):
        super().__init__("https://pcbuilders.lk", "PCBuilders")

    def get_categories(self) -> List[dict]:
        """Fetches categories from the homepage."""
        print("Fetching categories from pcbuilders.lk...")
        ...
        # Categories are often under a "Categories" heading or in specific sections
        cat_links = soup.select('a[href*="pcbuilders.lk/"]')
        ...
        for link in cat_links:
            ...
            # Filter for category-like URLs (avoiding cart, my-account, etc.)
            if url and url not in seen_urls and name:
                exclude_keywords = ['cart', 'my-account', 'checkout', 'contact', 'about', 'wishlist']
                if not any(kw in url for kw in exclude_keywords) and len(name) > 2:
    ...
    def crawl(self, categories: List[str] = None) -> Generator[Product, None, None]:
        ...
        print(f"Discovered {len(available_categories)} potential categories on PCBuilders.")
        ...
        if categories:
            ...
            # Focus on known component categories
            component_keywords = ['pc-components', 'mouse', 'keyboard', 'headset', 'processor', 'motherboard', 'gpu', 'ram', 'storage']
            for cat_data in available_categories:
                if any(kw in cat_data['slug'] for kw in component_keywords):
                    targets.append(cat_data)
        ...
        for cat_data in targets:
            ...
            print(f"Crawling PCBuilders category: {cat_name}...")
            ...
            while url:
                ...
                soup = BeautifulSoup(response.content, 'html.parser')
                # PCBuilders uses WooCommerce/Astra theme
                ...
                for p_item in products:
                    try:
                        ...
                        # Price
                        # Prices are often in <span class="price"> with <bdi> inside
                        ...
                        # Sometimes price might be "Rs. 10,000.00" - we need the number
                        import re
                        ...
                        # Availability
                        stock_tag = p_item.select_one('.out-of-stock') or \
                                    p_item.select_one('.ast-shop-product-out-of-stock')
                        ...
                        print(f"Error parsing product on PCBuilders: {e}")
                        continue
    ...
    def scrape_product_details(self, url: str) -> dict:
        ...
        # PCBuilders specifics (WooCommerce)
