...
export default function BuildPage() {
  ...
  const handleOwnedDetailChange = async (part, value) => {
    ...
    setSelectedIndex(-1); // Reset keyboard nav on typing
    ...
    try {
      ...
      setSearchResults({ ...searchResults, [part]: results });
    } finally {
  ...
  const renderComponentSearch = (part) => {
    return (
      ...
      >
        ...
        {/* Dropdown List - Only show if Active AND (has results OR is loading) */}
        {activeSearchPart === part && (loadingState[part] || (searchResults[part] && searchResults[part].length > 0)) && (
          <ul style={{
            listStyle: "none",
            padding: "0",
            margin: "4px 0 0",
              ...
              searchResults[part].map((res, index) => {
                ...
                return (
                  <li
                    ...
                    onMouseDown={(e) => {
                      // Use onMouseDown instead of onClick to prevent blur/focus race conditions
                      e.preventDefault();
                      selectComponent(part, displayName);
                    }}
                    style={{
                    ...
                    onMouseEnter={(e) => {
                      // Optional: Sync mouse hover with index if desired
